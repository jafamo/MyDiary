...
- Los logs JSON de PHP aplanan `context`/`extra` al nivel raíz (`App\Logger\FlattenedContextJsonFormatter`). Los del canal `messenger` llevan además un campo `status` (`App\Logger\MessengerStatusProcessor`) con el estado del mensaje: `received`, `sent`, `handled`, `no_handler`, `acknowledged` (procesado con éxito), `retry`, `failed` (descartado tras agotar reintentos) o `rejected` (enviado al transporte de fallos). Se deduce del texto del log de Symfony Messenger; los logs de `messenger` que no encajan en ningún estado (p. ej. `Stopping worker.`) no lo llevan.
