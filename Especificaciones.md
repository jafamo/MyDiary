...
4. Llama a Ollama para generar: (a) resumen del día, (b) lista de temas tratados, (c) leyenda de los emojis usados en el resumen.
...
- Los logs JSON de PHP aplanan `context`/`extra` al nivel raíz (`App\Logger\FlattenedContextJsonFormatter`). Los del canal `messenger` llevan además un campo `messenger_status` (`App\Logger\MessengerStatusProcessor`) con el estado del mensaje: `received`, `sent`, `handled`, `no_handler`, `acknowledged` (procesado con éxito), `retry`, `failed` (descartado tras agotar reintentos) o `rejected` (enviado al transporte de fallos). Se deduce del texto del log de Symfony Messenger; los logs de `messenger` que no encajan en ningún estado (p. ej. `Stopping worker.`) no lo llevan. No se llama `status` porque ese campo ya es numérico en el índice de Filebeat (código HTTP de nginx) y un valor de texto haría que Elasticsearch rechazara el documento.
- Cada petición HTTP a Symfony deja una línea en el canal `http` (`App\EventListener\HttpRequestLogListener`, en `kernel.terminate`) con `event: http.request`, `status` (código HTTP numérico, el mismo campo que el access log de nginx), `method`, `route`, `path` y `duration_ms`. Nivel `info` para < 400, `warning` para 4xx y `error` para 5xx. Se excluyen las rutas internas de Symfony (`/_wdt`, `/_profiler`...).
- Eventos de negocio en el canal `app`: `audio_recording.received` (webhook de Telegram; `result` = `created`, `duplicate_message`, `duplicate_file` o `retrying_after_error`, más el `status` HTTP devuelto) y `transcription.created` (worker; `audio_recording_id`, `transcription_id`, `audio_recording_status`, `processing_ms`, `model`).
- En `prod` el handler principal es `fingers_crossed` (solo escribe el buffer si hay un error en la misma petición/mensaje). Los canales `http`, `app` y `messenger` van en cambio a un handler `structured` sin buffer (`level: info`, mismo fichero), para que los INFO lleguen siempre a Kibana; están excluidos del `fingers_crossed` para no duplicarse.
