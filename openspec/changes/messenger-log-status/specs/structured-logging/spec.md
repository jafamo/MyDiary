...
Los registros de log del canal `messenger` SHALL incluir un campo `status` de primer nivel en el JSON con el estado del mensaje en su ciclo de vida, deducido del log que emite Symfony Messenger:

| Log de Symfony Messenger | `status` |
|---|---|
...
- **THEN** la línea JSON del log incluye `"status": "failed"`

### Requirement: Sin `status` en logs no reconocidos
El campo `status` SHALL añadirse solo a registros del canal `messenger` que correspondan a uno de los estados definidos; el resto de registros (otros canales, o logs de `messenger` como `Stopping worker.`) MUST quedar sin modificar.
...
- **THEN** la línea JSON del log no incluye campo `status`
