...
**Goals:**
- Campo `status` filtrable en Kibana para los logs de Messenger, sin tocar el código de Symfony ni duplicar líneas de log.

**Non-Goals:**
- Añadir `status` a logs propios de la app (`app`) o a otros canales.
- Cambiar la configuración de Filebeat o el mapeo de Elasticsearch.
- Métricas/contadores de mensajes (esto es solo logging).

## Decisions

- **Processor de Monolog limitado al canal `messenger`** (`#[AsMonologProcessor(channel: 'messenger')]`) que escribe `extra.status`. El formatter ya lo sube a la raíz. Alternativa descartada: un `EventSubscriber` sobre `WorkerMessageReceivedEvent` / `WorkerMessageHandledEvent` / `WorkerMessageFailedEvent` que emitiera líneas propias con `status`: duplicaría cada evento en el log y no cubriría `sent`/`no_handler`.
...
- **`extra` y no `context`**: si el contexto ya trae una clave `status`, `FlattenedContextJsonFormatter` da prioridad al contexto y no se pisa.
...
- [Symfony cambia el texto de sus logs en una actualización] → el registro queda sin `status` (no rompe nada); los tests unitarios fijan los textos actuales y la regresión se detecta revisando Kibana tras actualizar Symfony.
