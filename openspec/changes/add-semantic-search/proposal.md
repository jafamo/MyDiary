...
- La búsqueda genera el embedding de la consulta y ordena los resultados por distancia coseno usando `pgvector` (nueva extensión de PostgreSQL).
...
- `semantic-search`: búsqueda en lenguaje natural sobre transcripciones, basada en embeddings vectoriales y similitud coseno vía pgvector.
...
- Nuevo controlador/vista de Búsqueda + ruta en la navegación.
- `docker-compose.yml`: imagen de `diary-postgres`.
- Migración Doctrine para `pgvector` y la columna.
